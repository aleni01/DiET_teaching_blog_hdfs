...
interface ControlsProps {
  ...
  files: FileData[];
  updateFileProperty: (fileId: string, property: keyof FileData, value: any) => void;
  onReset: () => void;
}
    ...
    <div className="bg-white p-6 rounded-2xl shadow-sm border border-black/5 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div className="space-y-6">
          <h3 className="text-[15px] font-bold uppercase tracking-widest text-zinc-900">Global Controls</h3>
            ...
            <div>
              ...
              <div className="flex justify-between mt-2 text-[15px] text-zinc-400 font-mono uppercase tracking-wider">
                {Array.from({length: 4}).map((s, i) => (
                  <span key={i} className={(i+1) === numFiles ? 'text-zinc-900 font-bold' : ''}>{i+1}</span>
                ))}
        ...
        {/* Column 2: Files 1 & 2 */}
        <div className="space-y-6">
          <h3 className="text-[15px] font-bold uppercase tracking-widest text-zinc-900">Files 1 & 2</h3>
          <div className="space-y-8">
            {files.slice(0, 2).map((file, idx) => (
              <FileControl key={file.id} file={file} idx={idx} updateFileProperty={updateFileProperty} />
            ))}
          </div>
        </div>
        ...
        {/* Column 3: Files 3 & 4 */}
        <div className="space-y-6">
          <h3 className="text-[15px] font-bold uppercase tracking-widest text-zinc-900">Files 3 & 4</h3>
          <div className="space-y-8">
            {files.slice(2, 4).map((file, idx) => (
              <FileControl key={file.id} file={file} idx={idx + 2} updateFileProperty={updateFileProperty} />
            ))}
            {files.length < 3 && (
              <div className="h-full flex items-center justify-center border-2 border-dashed border-zinc-100 rounded-xl p-8">
                <p className="text-xs text-zinc-400 italic">Add more files to see controls</p>
              </div>
            )}
          </div>
        </div>
  ...
  return (
    <div className="space-y-4 p-4 bg-zinc-50 rounded-xl border border-zinc-100">
      <div className="flex justify-between items-center">
        <label className="text-[15pt] font-bold flex items-center gap-2" style={{ color: `hsl(${file.colorHue}, 70%, 40%)` }}>
      ...
      <div className="space-y-1">
        ...
        <input 
              ...
              <div className="flex justify-between mt-2 text-[15px] text-zinc-400 font-mono uppercase tracking-wider">
                {Array.from({length: 4}).map((s, i) => (
                  <span key={i} className={(i+1) === file.replicationFactor ? 'text-zinc-900 font-bold' : ''}>{i+1}</span>
                ))}
        ...
        <input 
              ...
              <div className="flex justify-between mt-2 text-[15px] text-zinc-400 font-mono uppercase tracking-wider">
                {Array.from({length: 10}).map((s, i) => (
                  <span key={i} className={(i+1) === Number(file.clientNodeId.split('-')[1])  ? 'text-zinc-900 font-bold' : ''}>{i+1}</span>
                ))}
